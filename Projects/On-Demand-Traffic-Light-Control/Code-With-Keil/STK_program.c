...
Std_ReturnType STK_SetBusyWait(u32 Copy_Microseconds)
{
   ...
   * (STK_AHB_CLK / 1000000): This part of the formula converts the system clock frequency from Hertz (cycles per second) to cycles per microsecond.
   * TicksRequired is number of cycles (Ticks) required (microsecond * (cycles / microsecond) = cycles).
   * */ 
  u32 TicksRequired = (u32)(Copy_Microseconds * (STK_AHB_CLK / 1000000));

  /**< Check if the ticks required is within the valid range */
  if (TicksRequired <= STK_MAX_TIME_VAL) 
  {
...
Std_ReturnType STK_SetDelay_ms(f32 Copy_Milliseconds)
{
  ...
  u32 TicksRequired = (u32)(Copy_Milliseconds * (STK_AHB_CLK / 1000));
